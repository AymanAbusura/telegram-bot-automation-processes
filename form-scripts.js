...
$(document).ready(function() {
    ...
    const errors = {};
    const submitBtn = $('#form button[type="submit"], #form input[type="submit"]');

    inputs.forEach(input => {
        errors[input.getAttribute('name')] = true;
    });
    ...
    function validateInput(input) {
        ...
        if (fieldName === 'phone' || fieldName === 'phone_visible') {
            if (!iti.isValidNumber()) showError(input, errorDiv, fieldName);
            else clearError(input, errorDiv, fieldName);
        }
        else if (fieldName === 'email') {
            if (!/^\\S+@\\S+\\.[A-Za-z]{2,}$/.test(value)) showError(input, errorDiv, fieldName);
            else clearError(input, errorDiv, fieldName);
        }
        else if (fieldName === 'first_name' || fieldName === 'last_name') {
            if (/\\d/.test(value) || value.length < 2) showError(input, errorDiv, fieldName);
            else clearError(input, errorDiv, fieldName);
        }
        else {
            if (value.length < 2) showError(input, errorDiv, fieldName);
            else clearError(input, errorDiv, fieldName);
        }
    ...
    inputPhone.addEventListener('countrychange', () => validateInput(inputPhone));
    ...
    $('#form').on('submit', function(e) {
        ...
        const loader = document.querySelector('.rf-form__loader');
